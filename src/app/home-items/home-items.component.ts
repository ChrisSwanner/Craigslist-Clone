    ...
    ngOnInit() {
      this.saleService.getSales().subscribe(dataLastEmittedFromObserver => {
        ...
        for (let i = 0; i < this.saleDisplay.length; i++) {
          if (this.saleDisplay[i].category != "Vehicles" && this.saleDisplay[i].category != "Appliances" &&  this.saleDisplay[i].category != "Electronics" &&  this.saleDisplay[i].category != "Jewelry" &&  this.saleDisplay[i].category != "Cell Phones" ) {
